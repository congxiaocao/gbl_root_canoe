...
|----|----------|--------|-------------|----------------|
| **Patch 1** | `patch_abl_gbl` | ABL binary | Replaces UTF-16 `"efisp"` → `"nulls"` in ABL. | Prevents ABL from recursively loading efisp (which would overwrite the patched ABL with the original). |
...
| 编号 | 函数 | 目标 | 补丁内容 | 失败影响 |
|------|------|------|---------|---------|
| **补丁 1** | `patch_abl_gbl` | ABL 二进制 | 将 UTF-16 `"efisp"` 替换为 `"nulls"`。 | 防止 ABL 递归加载 efisp（会用原始 ABL 覆盖已补丁的 ABL）。 |
